...
def beep(freq, dur, vol=1.0):
    return resource.createAudio([
        math.sin(2 * math.pi * freq * n / 44100) * vol
...
def input(evt, dev, id, val, v2):
    ...
    if state == "over" and evt == "button" and val > 0:
        ...
        score = 0
        state = "play"
        player['x'] = int(winSzX // 2)
        player['vx'] = 0
    elif state == "play" and evt == "axis" and id == 0:
...
def update(dt):
    ...
    if random.random() < 0.01 + score * 0.001:
        blocks.append([
            random.randint(0, winSzX - int(20 * sc)),
            -int(20 * sc) ])
...
def draw(gfx):
    ...
    for b in blocks:
        gfx.fillRect(b[0], b[1], 20 * sc, 20 * sc)
    gfx.color(0xffffffaa)
    gfx.fillText(font, 10 * sc, 20 * sc, f"Score: {score}  High: {highscore}")
    if state == "over":
