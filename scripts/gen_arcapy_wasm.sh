#!/bin/sh
# Builds the WASM-compiled arcapy (pocketpy) language driver for the browser
...
emcc "$BASEDIR/bindings_arcapy_wasm.c" "$BASEDIR/../external/pocketpy.c" \
	-I"$BASEDIR/../external" \
	-DPK_IS_PUBLIC_INCLUDE -DPK_ENABLE_THREADS=0 \
	-s WASM=1 -s MODULARIZE=1 -s EXPORT_NAME=createArcapyModule \
	...
	-O2 \
	-o "$BASEDIR/../browser_runtime/arcapy.js"
